...
#include "../LuaLibraries/LuaNetwork.hpp"
...
namespace {
	...
	static void _LoadLuaFileInTable(const std::filesystem::directory_entry& Entry, int TableIDX) {
		...
		lua_pushstring(Game::Lua::State, Path.filename().generic_u8string().c_str());
		lua_pushvalue(Game::Lua::State, -1);

		unlikely_branch
		//if (lua_getfield(Game::Lua::State, -2, Path.filename().generic_u8string().c_str()) == LUA_TNIL) {
		if (lua_rawget(Game::Lua::State, TableIDX) != LUA_TNIL) {
			std::clog << Path << " already has an entry, skipping" << std::endl;
			goto _Cleanup;
		}
		lua_settop(Game::Lua::State, TableIDX + 1);

		unlikely_branch
		if (LuaHelper::LoadFileLog(Game::Lua::State, Path.generic_u8string().c_str()) != LUA_OK) {
			goto _Cleanup;
		}
	...
	static void _LoadLuaAssetDirectoryInTable(const char* DirectoryPath) {
		...
		if (!std::filesystem::exists(FilesystemPath)) {
			std::cerr << "Directory " << FilesystemPath.generic_string() << " doesn't exist." << std::endl;
		...
		if (!std::filesystem::is_directory(FilesystemPath)) {
			std::cerr << "Path " << FilesystemPath.generic_string() << " isn't a directory." << std::endl;
...
void Game::Lua::Init() {
	...
	Game::Lua::GameTable = LuaHelper::StackTableReference(State, 0, 4);
