...
#include "LuaGraphics_Program.hpp"
...
void Game::Lua::CLibraries::Graphics::Program::Init(
    ...
    ProgramTable.SetKeyClosure(State, Program::SetUniformSint32,
                               "SetUniformSint32");
    ProgramTable.SetKeyClosure(State, Program::SetUniformUint32,
                               "SetUniformUint32");

    // TODO: change this to use a SetUniformBool thing
    ProgramTable.SetKeyClosure(State, Program::SetUniformSint32,
                               "SetUniformBool");
    ProgramTable.SetKeyClosure(State, Program::SetUniformFloat,
                               "SetUniformFloat");

    ProgramTable.SetKeyClosure(State, Program::SetUniformIvec2,
                               "SetUniformIvec2");
    ProgramTable.SetKeyClosure(State, Program::SetUniformUvec2,
                               "SetUniformUvec2");
    ProgramTable.SetKeyClosure(State, Program::SetUniformBvec2,
                               "SetUniformBvec2");
    ProgramTable.SetKeyClosure(State, Program::SetUniformFvec2,
                               "SetUniformVec2");

    ProgramTable.SetKeyClosure(State, Program::SetUniformIvec3,
                               "SetUniformIvec3");
    ProgramTable.SetKeyClosure(State, Program::SetUniformUvec3,
                               "SetUniformUvec3");
    ProgramTable.SetKeyClosure(State, Program::SetUniformBvec3,
                               "SetUniformBvec3");
    ProgramTable.SetKeyClosure(State, Program::SetUniformFvec3,
                               "SetUniformVec3");

    ProgramTable.SetKeyClosure(State, Program::SetUniformIvec4,
                               "SetUniformIvec4");
    ProgramTable.SetKeyClosure(State, Program::SetUniformUvec4,
                               "SetUniformUvec4");
    ProgramTable.SetKeyClosure(State, Program::SetUniformBvec4,
                               "SetUniformBvec4");
    ProgramTable.SetKeyClosure(State, Program::SetUniformFvec4,
                               "SetUniformVec4");

    ProgramTable.SetKeyClosure(State, Program::SetUniformMat2,
                               "SetUniformMat2");
    ProgramTable.SetKeyClosure(State, Program::SetUniformMat3,
                               "SetUniformMat3");
    ProgramTable.SetKeyClosure(State, Program::SetUniformMat4,
                               "SetUniformMat4");
...
int Game::Lua::CLibraries::Graphics::Program::__new(lua_State* State) {
    Classes::Program* ProgramUD = static_cast<Classes::Program*>(
...
int Game::Lua::CLibraries::Graphics::Program::SetUniformIvec2(
    lua_State* State) {
    if (const void* VecUD = luaL_testudata(State, 2, "IVector2");
        VecUD == NULL) {
...
int Game::Lua::CLibraries::Graphics::Program::SetUniformUvec2(
    lua_State* State) {
    if (const void* VecUD = luaL_testudata(State, 2, "UVector2");
        VecUD == NULL) {
...
int Game::Lua::CLibraries::Graphics::Program::SetUniformFvec2(
    lua_State* State) {
    if (const void* VecUD = luaL_testudata(State, 2, "Vector2");
        VecUD == NULL) {
...
int Game::Lua::CLibraries::Graphics::Program::SetUniformIvec3(
    lua_State* State) {
    if (const void* VecUD = luaL_testudata(State, 2, "IVector3");
        VecUD == NULL) {
...
int Game::Lua::CLibraries::Graphics::Program::SetUniformUvec3(
    lua_State* State) {
    if (const void* VecUD = luaL_testudata(State, 2, "UVector3");
        VecUD == NULL) {
...
int Game::Lua::CLibraries::Graphics::Program::SetUniformFvec3(
    lua_State* State) {
    if (const void* VecUD = luaL_testudata(State, 2, "Vector3");
        VecUD == NULL) {
...
int Game::Lua::CLibraries::Graphics::Program::SetUniformIvec4(
    lua_State* State) {
    if (const void* VecUD = luaL_testudata(State, 2, "IVector4");
        VecUD == NULL) {
...
int Game::Lua::CLibraries::Graphics::Program::SetUniformUvec4(
    lua_State* State) {
    if (const void* VecUD = luaL_testudata(State, 2, "UVector4");
        VecUD == NULL) {
...
int Game::Lua::CLibraries::Graphics::Program::SetUniformFvec4(
    lua_State* State) {
    if (const void* VecUD = luaL_testudata(State, 2, "Vector4");
        VecUD == NULL) {
...
int Game::Lua::CLibraries::Graphics::Program::SetUniformMat2(lua_State* State) {
    ...
    if (Mat2UD = static_cast<Math::Mat2*>(luaL_testudata(State, 2, "Mat2"));
        Mat2UD == NULL) {
...
int Game::Lua::CLibraries::Graphics::Program::SetUniformMat3(lua_State* State) {
    ...
    if (Mat3UD = static_cast<Math::Mat3*>(luaL_testudata(State, 2, "Mat3"));
        Mat3UD == NULL) {
...
int Game::Lua::CLibraries::Graphics::Program::SetUniformMat4(lua_State* State) {
    ...
    if (Mat4UD = static_cast<Math::Mat4*>(luaL_testudata(State, 3, "Mat4"));
        Mat4UD == NULL) {
...
int Game::Lua::CLibraries::Graphics::Classes::Program::AttachShader(
    ...
    const Classes::Shader* ShaderUD =
        static_cast<Classes::Shader*>(luaL_testudata(State, 2, "Shader"));
    if (ShaderUD == NULL) {
        luaL_error(
            State,
