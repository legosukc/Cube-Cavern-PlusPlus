...
namespace Game::Lua::CLibraries::Matrix {
    ...
    namespace ClassFuncs {
        ...
        static int __new(lua_State* State) {
            ...
            if (Row1 = static_cast<typename MatrixType::ColumnType*>(
                    LuaHelper::TestMetatable(State, 1, lua_upvalueindex(1)));
                Row1 == NULL) {
                lua_settop(
                    State,
                    (MatrixType::RowCount * MatrixType::ColumnCount) + 1);

                for (int i = 2;
                     i <= MatrixType::RowCount * MatrixType::ColumnCount; ++i) {
                    reinterpret_cast<
                        typename MatrixType::ColumnType::ComponentType*>(
                        NewMatrix)[i - 2] = luaL_optnumber(State, i, 0.0);
                }
                ...
                for (int i = 3; i <= MatrixType::ColumnCount; ++i) {
                    (*NewMatrix)[i - 2] =
                        *static_cast<typename MatrixType::ColumnType*>(
                            LuaHelper::TestMetatable(State, i,
                                                     lua_upvalueindex(1)));
                }
            }
        ...
        static int __index(lua_State* State) {
            ...
            MatrixUD = static_cast<MatrixType*>(
                LuaHelper::CheckMetatable(State, 1, lua_upvalueindex(1)));
            Index = luaL_checkinteger(State, 2);

            unlikely_branch if (Index < 0 ||
                                Index > MatrixType::ColumnCount - 1) {
                lua_getfield(State, lua_upvalueindex(1), "__name");
                luaL_error(State, "Cannot index ", lua_tostring(State, -1),
                           ": Numerical index out of range: (", Index, ").");
            }

            LuaHelper::Push<typename MatrixType::ColumnType>(
                State, (*MatrixUD)[Index]);
            return 1;
        }
        ...
        static int __newindex(lua_State* State) {
            ...
            (*MatrixUD)[Index] = *static_cast<typename MatrixType::ColumnType*>(
                LuaHelper::CheckMetatable(State, 3, lua_upvalueindex(2)));
            return 0;
        }
        ...
        static int __mul(lua_State* State) {
            ...
            *Result = *static_cast<MatrixType*>(LuaHelper::CheckMetatable(
        ...
        static int __eq(lua_State* State) {
            ...
            A = static_cast<MatrixType*>(
                LuaHelper::TestMetatable(State, 1, lua_upvalueindex(1)));
            B = static_cast<MatrixType*>(
                LuaHelper::TestMetatable(State, 2, lua_upvalueindex(1)));

            lua_pushboolean(State,
                            !(A == NULL || B == NULL) && A->operator==(*B));
            return 1;
        }
    }

    template <class MatrixType>
    void SetupMatrix(lua_State* State,
        ...
        // Metatable.PushReference(State);
        // lua_copy(State, Metatable.GetStackIndex() + 1,
        // Metatable.GetStackIndex() + 6);

        Metatable.PushReference(State);
        Metatable.SetKeyClosure(State, ClassFuncs::__tostring<MatrixType>,
                                "__tostring", 1);

        Metatable.PushReference(State);
        Metatable.SetKeyClosure(State, ClassFuncs::__index<MatrixType>,
                                "__index", 1);

        Metatable.PushReference(State);
        luaL_getmetatable(State, VectorName);
        Metatable.SetKeyClosure(State, ClassFuncs::__newindex<MatrixType>,
                                "__newindex", 2);

        Metatable.PushReference(State);
        Metatable.SetKeyClosure(State, ClassFuncs::__mul<MatrixType>, "__mul",
                                1);

        Metatable.PushReference(State);
        Metatable.SetKeyClosure(State, ClassFuncs::__eq<MatrixType>, "__eq", 1);

        LibraryTable.SetKeyClosure(State, ClassFuncs::__new<MatrixType>, "new",
                                   1);
    ...
    static int _LookAt(lua_State* State) {
        ...
        *Result = Math::Transform::LookAt(
            *static_cast<Math::Vector3*>(
                luaL_checkudata(State, 1, "Vector3")),  // Eye
            *static_cast<Math::Vector3*>(
                luaL_checkudata(State, 2, "Vector3")),  // Center
            *static_cast<Math::Vector3*>(
                luaL_checkudata(State, 3, "Vector3"))  // Up
        );
    ...
    static int _Perspective(lua_State* State) {
        ...
        *Result = Math::Transform::Perspective(
            ...
            //*static_cast<zKevin*>(luaL_checkudata(State, 5, "zKevin"))
        );
...
void Game::Lua::CLibraries::Matrix::Init(lua_State* State) {
    ...
    Game::Lua::CLibraries::Matrix::SetupMatrix<Math::Mat4>(State, "Vector4",
                                                           "Mat4");

    lua_getglobal(State, "Mat4");

    lua_pushcfunction(State, Game::Lua::CLibraries::Matrix::_LookAt,
                      "Mat4.LookAt");
