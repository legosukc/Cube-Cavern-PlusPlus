...
namespace Utils {
    ...
    template <typename... ConnectionArgs_T>
    class Event {
       public:
        Event() = default;
        ~Event();
...
template <typename... ConnectionArgs_T>
Utils::Event<ConnectionArgs_T...>::~Event() {
    Connection *NextConnection, *CurrentConnection;
...
template <typename... ConnectionArgs_T>
typename Utils::Event<ConnectionArgs_T...>::Connection*
    Utils::Event<ConnectionArgs_T...>::Connect(
        ConnectionFuncType ConnectionFunc) {
    Connection* NewConnection = new Connection;
    NewConnection->_ConnectionFunc = ConnectionFunc;
    NewConnection->_NextConnection = NULL;

    unlikely_branch if (this->_BackConnection == NULL) {
...
template <typename... ConnectionArgs_T>
void Utils::Event<ConnectionArgs_T...>::RemoveConnection(
    ...
    if (_Connection == this->_BackConnection) {
        ...
        _Connection->_PreviousConnection->_NextConnection =
            _Connection->_NextConnection;
        _Connection->_NextConnection->_PreviousConnection =
            _Connection->_PreviousConnection;
    }
...
template <typename... ConnectionArgs_T>
void Utils::Event<ConnectionArgs_T...>::Fire(
    ConnectionArgs_T... Args) const {
