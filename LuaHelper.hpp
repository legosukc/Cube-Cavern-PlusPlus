...
namespace LuaHelper {
	...
	struct StackTableReference {
		...
		// Uses the top of the passed States' stack as the table reference (TableIndex).
		inline StackTableReference(lua_State* State, const char* MetatableName) : TableIndex(0) {
			luaL_newmetatable(State, MetatableName);
			*const_cast<int*>(&this->TableIndex) = lua_gettop(State);
		}

		// Creates a new table and sets the table reference (TableIndex) to the top of the stack.
		inline StackTableReference(lua_State* State, int AllocateArrays = 0, int AllocateKeys = 0) : TableIndex(0) {
			lua_createtable(State, AllocateArrays, AllocateKeys);
			*const_cast<int*>(&this->TableIndex) = lua_gettop(State);
		}

		// Uses the passed 'Index' parameter as the table reference (TableIndex).
		constexpr StackTableReference(int Index) : TableIndex(Index) {}
		...
		template<typename Index_T>
		inline void SetTableClosure(lua_State* State, lua_CFunction Closure, Index_T Index, int Upvalues) {
			LuaHelper::SetTableClosure<Index_T>(State, this->TableIndex, Closure, Index, Upvalues);
		}

		inline void SetKeyClosure(lua_State* State, lua_CFunction Closure, const char* Key, int Upvalues) {
			LuaHelper::SetKeyClosure(State, this->TableIndex, Closure, Key, Upvalues);
		}

		inline void SetIndexClosure(lua_State* State, lua_CFunction Closure, lua_Integer Index, int Upvalues) {
